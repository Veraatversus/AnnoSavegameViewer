...
namespace AnnoSavegameViewer.Serialization.Memory {
  ...
  public class MemoryReader {
    ...
    /// <summary> Gets the <see cref="ReadOnlySpan{Byte}"/> from which data is read. </summary>
    ...
    public TimeSpan ReadTimeSpan(TimeSpanPattern pattern = default, Endian endian = Endian.System, int offset = 0) {
      Memory.Span.Slice(Position).ReadTimeSpan(out var result, offset, pattern, endian);
      this.Position += 8;
      return result;
    }
    ...
    public object ReadObject(Type type, BinaryContentAttribute attribute = null) {
      ...
      if (type == typeof(string))
        ...
        return ReadTimeSpan(att.TimeSpanPattern, att.Endian, att.Offset);
        ...
        return ReadReadOnlyMemory(lenght, att.Offset);
      else
