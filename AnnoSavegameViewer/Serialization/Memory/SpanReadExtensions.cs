  ...
  public static class SpanReadExtensions {
    ...
    public static void ReadBoolean(this in ReadOnlySpan<byte> span, out bool result, out int usedBytes, int offset = 0, BoolPattern pattern = default) {
      (result, usedBytes) = pattern switch
      {
        ...
        BoolPattern.Word => (MemoryMarshal.Read<UInt16>(span.Slice(offset)) != 0, 2),
        BoolPattern.Dword => (MemoryMarshal.Read<UInt32>(span.Slice(offset)) != 0, 4)
      };
    }
    ...
    public static void ReadDecimal(this in ReadOnlySpan<byte> span, out Decimal result, int offset = 0) {
      result = MemoryMarshal.Read<Decimal>(span.Slice(offset));
    }
    ...
    public static void ReadInt16(this in ReadOnlySpan<byte> span, out Int16 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<Int16>(span.Slice(offset)))
          : MemoryMarshal.Read<Int16>(span.Slice(offset));
    }
    ...
    public static void ReadInt32(this in ReadOnlySpan<byte> span, out Int32 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<Int32>(span.Slice(offset)))
          : MemoryMarshal.Read<Int32>(span.Slice(offset));
    }
    ...
    public static void ReadInt64(this in ReadOnlySpan<byte> span, out Int64 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<Int64>(span.Slice(offset)))
          : MemoryMarshal.Read<Int64>(span.Slice(offset));
    }
    ...
    public static void ReadUInt16(this in ReadOnlySpan<byte> span, out UInt16 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<UInt16>(span.Slice(offset)))
          : MemoryMarshal.Read<UInt16>(span.Slice(offset));
    }
    ...
    public static void ReadUInt32(this in ReadOnlySpan<byte> span, out UInt32 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<UInt32>(span.Slice(offset)))
          : MemoryMarshal.Read<UInt32>(span.Slice(offset));
    }
    ...
    public static void ReadUInt64(this in ReadOnlySpan<byte> span, out UInt64 result, int offset = 0, Endian endian = Endian.System) {
      result = !endian.IsSystem()
          ? BinaryPrimitives.ReverseEndianness(MemoryMarshal.Read<UInt64>(span.Slice(offset)))
          : MemoryMarshal.Read<UInt64>(span.Slice(offset));
    }

    public static T ReadObject<T>(this ReadOnlyMemory<byte> memory) => (T)ReadObject(memory, typeof(T));
    ...
    public static object ReadObject(this in ReadOnlyMemory<byte> memory, Type type, BinaryContentAttribute attribute = null) {
      ...
      if (type == typeof(object)) {
        ...
        var lenght = att.Length != 0 ? att.Length : memory.Length - att.Offset;
        return memory.Span.ReadString(att.Offset, lenght, encoding, att.StringPattern);
      }
      else if (type.IsEnumerable())
        throw new InvalidOperationException("Multidimensional arrays cannot be read directly.");
      else if (type == typeof(bool))
        return memory.Span.ReadBoolean(att.Offset, att.BoolPattern);
      else if (type == typeof(byte))
        return memory.Span.ReadByte(att.Offset);
      else if (type == typeof(DateTime))
        return memory.Span.ReadDateTime(att.Offset, att.DateTimePattern, att.Endian);
      else if (type == typeof(decimal))
        return memory.Span.ReadDecimal(att.Offset);
      else if (type == typeof(double))
        return memory.Span.ReadDouble(att.Offset, att.Endian);
      else if (type == typeof(short))
        return memory.Span.ReadInt16(att.Offset, att.Endian);
      else if (type == typeof(int))
        return memory.Span.ReadInt32(att.Offset, att.Endian);
      else if (type == typeof(long))
        return memory.Span.ReadInt64(att.Offset, att.Endian);
      else if (type == typeof(sbyte))
        return memory.Span.ReadSByte(att.Offset);
      else if (type == typeof(float))
        return memory.Span.ReadSingle(att.Offset, att.Endian);
      else if (type == typeof(ushort))
        return memory.Span.ReadUInt16(att.Offset, att.Endian);
      else if (type == typeof(uint))
        return memory.Span.ReadUInt32(att.Offset, att.Endian);
      else if (type == typeof(ulong))
        return memory.Span.ReadUInt64(att.Offset, att.Endian);
      else if (type.IsEnum)
        return memory.Span.ReadEnum(type, att.Strict, att.Offset, att.Endian);
      else
        return memory.Slice(att.Offset).ToReader().ReadCustomObject(type);
    }
    ...
    private static void DecodeString(this in ReadOnlySpan<byte> span, out String result, out int usedBytes, int length, Encoding encoding, int offset = 0) {
      ...
      decoder.Convert(span.Slice(offset), chars, true, out int bytesUsed, out int charsUsed, out bool completed);
      usedBytes = bytesUsed;
