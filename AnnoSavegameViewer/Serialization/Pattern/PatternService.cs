namespace AnnoSavegameViewer.Serialization.Pattern {
  ...
  public class PatternService {
    ...
    public PropertyPattern GetValueOrDefault(TreeNode node) {
      ...
      if (node.Guid == default) {
        ...
        try {
        propertyPattern.RegisteredNodes.Add(node.Guid, new WeakReference<TreeNode>(node));
        }
        catch (Exception e) {
    ...
    public Task SaveCSharpClasses() {
      ...
      return Task.Run(() => {
        ...
        foreach (var csClass in Classes.Values.OrderByDescending(c => c.Pattern.ClassName.Length)) {
          //Standard Usings
          _ = csClass.Usings.Add("AnnoSavegameViewer.Serialization.Core");
          _ = csClass.Usings.Add("AnnoSavegameViewer.Structures.DataTypes");
          _ = csClass.Usings.Add("System");

          //Namespace
          csClass.Namespace = $"AnnoSavegameViewer.Structures.{Name}.Generated";
