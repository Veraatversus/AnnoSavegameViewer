...
namespace CSharpCode.SharpZipLib.Zip.Compression.Streams {
  using System;
  using System.IO;
    ...
    public ICryptoTransform CryptoTransform {
      set {
        ...
        if (cryptoTransform != null) {
          ...
          if (Available > 0) {
            _ = cryptoTransform.TransformBlock(rawData, rawLength - Available, Available, clearText, rawLength - Available);
          }
        }
        else {
          clearText = rawData;
  ...
  /// as the <see cref="CSharpCode.SharpZipLib.GZip.GZipInputStream">GZipInputStream</see>.
  ///
  ...
  public class InflaterInputStream : Stream {
    ...
    public long Skip(long count) {
      ...
      if (baseInputStream.CanSeek) {
        _ = baseInputStream.Seek(count, SeekOrigin.Current);
        return count;
      }
      else {
        var length = 2048;
