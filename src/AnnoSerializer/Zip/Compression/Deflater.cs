...
namespace CSharpCode.SharpZipLib.Zip.Compression {
  ...
  /// <summary>
  /// This is the Deflater class. The deflater class compresses input with the deflate algorithm
  /// described in RFC 1951. It has several compression levels and three different strategies
  /// described below. /// This class is <i> not </i> thread safe. This is inherent in the API, due
  /// to the split of deflate and setInput. /// author of the original java version : Jochen Hoenicke
  /// </summary>
  public class Deflater {
    ...
    /// <summary> Returns true if the stream was finished and no more output bytes are available. </summary>
    ...
    /// <summary> Gets the current adler checksum of the data that was processed so far. </summary>
    ...
    /// true, if we should suppress the Zlib/RFC1950 header at the beginning and the adler checksum
    /// at the end of the output. This is useful for the GZIP/PKZIP formats.
    /// </param>
    /// <exception cref="System.ArgumentOutOfRangeException"> if lvl is out of range. </exception>
    public Deflater(int level, bool noZlibHeaderOrFooter) {
      ...
      else if (level is < NO_COMPRESSION or > BEST_COMPRESSION) {
    ...
    /// indicates that more input is needed. If you call setInput when needsInput() returns false,
    /// the previous input that is still pending will be thrown away. The given byte array should
    /// not be changed, before needsInput() returns true again. This call is equivalent to <c>
    /// setInput(input, 0, input.length) </c>.
    /// </summary>
    ...
    /// <summary>
    /// Sets the compression level. There is no guarantee of the exact position of the change, but
    /// if you call this when needsInput is true the change of compression level will occur
    /// somewhere near before the end of the so far given input.
    /// </summary>
    ...
    /// <summary>
    /// Sets the compression strategy. Strategy is one of DEFAULT_STRATEGY, HUFFMAN_ONLY and
    /// FILTERED. For the exact position where the strategy is changed, the same as for SetLevel() applies.
    /// </summary>
    ...
    /// <summary> Deflates the current input block with to the given array. </summary>
    /// <param name="output"> The buffer where compressed data is stored </param>
    /// <returns>
    /// The number of compressed bytes added to the output, or 0 if either IsNeedingInput() or
    /// IsFinished returns true or length is zero.
    /// </returns>
    ...
    /// <returns>
    /// The number of compressed bytes added to the output, or 0 if either needsInput() or
    /// finished() returns true or length is zero.
    /// </returns>
    /// <exception cref="System.InvalidOperationException"> If Finish() was previously called. </exception>
    ...
    /// <c> setDictionary(dict, 0, dict.Length) </c>.
    /// </summary>
    ...
    /// <summary>
    /// Sets the dictionary which should be used in the deflate process. The dictionary is a byte
    /// array containing strings that are likely to occur in the data which should be compressed.
    ...
    /// <param name="index"> The index where dictionary information commences. </param>
    ...
    /// <summary> This level won't compress at all but output uncompressed blocks. </summary>
    public const int NO_COMPRESSION = 0;

    /// <summary>
    /// The compression method. This is the only method supported so far. There is no need to use
    ...
    /// <summary> Compression Level as an enum for safer use </summary>
    ...
    // private static int INIT_FINISHING_STATE = 0x08; private static int SETDICT_FINISHING_STATE = 0x09;
    ...
    /// <summary> If true no Zlib/RFC1950 headers or footers are generated </summary>
